...
graph TB
    ...
    subgraph "Ingest Layer"
        EP[POST /api/ingest]
        SF[Staleness Filter<br/>ignore stale events only when state is live]
        DD[Dedup<br/>UNIQUE device_id + seq]
        Q[asyncio.Queue<br/>50k capacity]
        BW[Batch Worker<br/>up to 500 events per drain]
    end
    ...
    subgraph "Presentation"
        ...
        AI[AI Briefing<br/>local LLM + template fallback]
    end
...
|------|-----------|-------------|-------|
| 1 | Ingest + queue | queue-dependent | The endpoint enqueues and returns immediately. |
| 2 | Batch write | queue-dependent | The worker drains up to 500 events at a time. |
| 3 | Debounce wait | 30,000ms | Configurable. Prevents flicker. |
| 4 | Pole state update | database-dependent | Pole state is updated after deduplication. |
| 5 | Topology build | ~100ms | MST on ~80 poles (O(n² log n)) |
| 6 | Boundary BFS | O(V + E) | Linear in the DT tree. |
| 7 | Grouping + dedup | candidate-dependent | Suppresses lower-level duplicates. |
| 8 | Confidence scoring | candidate-dependent | Named penalties are multiplied. |
| 9 | Incident upsert | database-dependent | Updates an overlapping active incident. |
| 10 | SSE broadcast | connection-dependent | Non-blocking fanout. |
| **Total** | **30s debounce + processing** | The <120s target is a design goal; no end-to-end p95 measurement is claimed. |
...
Interactive API documentation is generated by FastAPI and is available at
`/docs`. The deployed reference is https://gridwatch-api.onrender.com/docs.

| Method | Path | Purpose |
|---|---|---|
| POST | `/api/ingest`, `/api/ingest/batch` | Accept one or many telemetry events. |
| GET | `/api/dashboard/stats`, `/api/poles`, `/api/transformers`, `/api/feeders`, `/api/edges` | Populate the operator console and map. |
| GET | `/api/incidents`, `/api/incidents/active`, `/api/incidents/{id}` | Read tickets. |
| PATCH | `/api/incidents/{id}/status` | Advance a human-controlled ticket state. System-only states are rejected. |
| POST | `/api/incidents/{id}/briefing` | Produce an optional AI/template crew briefing. |
| GET | `/api/events/stream` | SSE stream used by the console for incident updates. |
| GET | `/api/scheduled-outages` | List scheduled-outage records. |
| POST | `/api/simulator/*` | Seed, inject span/DT/feeder/dead-sensor/scheduled-outage events, and repair an incident. |
| GET | `/api/health` | Deployment health check. |

## Operational Limits and Honest Performance Statement

The 30-second debounce is deliberate and leaves 90 seconds of the assignment's
120-second ticket-visibility target for queueing, database work, topology, and
UI delivery. The algorithmic costs above are derived from the implementation;
they are not benchmark measurements. The core engine suite (18 tests) passes,
but the included 5,000-event load test has not been run against the public
deployment. A production design would replace the in-process queue and SSE
fanout with durable/shared infrastructure before horizontally scaling.
